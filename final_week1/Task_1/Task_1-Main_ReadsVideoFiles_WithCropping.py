...
while True:
    ...
    if not ret:
        ...
        continue
    (h, w) = frame.shape[:2]
    # This gives the lower two-thirds of the frame so its more useful in practical purposes like the actual robot view of the path. Additional noise reduction and analysis is required since this is not upto the mark yet.
    cropped_frame = frame[int(h/3):h, 0:w]
    blur = cv2.GaussianBlur(cropped_frame,(5,5),cv2.BORDER_DEFAULT)
